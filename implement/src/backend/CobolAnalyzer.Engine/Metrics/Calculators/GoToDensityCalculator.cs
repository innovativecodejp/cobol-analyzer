...
public static class GoToDensityCalculator
{
    public static double Calculate(ProgramNode ast)
    {
        var procDiv = ast.Children.OfType<DivisionNode>()
            .FirstOrDefault(d => d.Name == "PROCEDURE DIVISION");
        if (procDiv == null) return 0.0;

        var allStmts = CollectAllStatements(procDiv).ToList();
        int total = allStmts.Count;
        if (total == 0) return 0.0;

        int gotoCount = allStmts.Count(s => s.StatementType == "GOTO");
        return (double)gotoCount / total;
    }
